...
| *(trip-planner plugin)* | The taste/planning tooling lives in the **`trip-planner`** Claude Code plugin (marketplace `trip-tools`, checked out at **`~/.claude/plugins/marketplaces/trip-tools/`** — skills live under `plugins/trip-planner/skills/<name>/SKILL.md`. It is its own git repo, **github.com/maklipsa/trip-planner**, branch **`master`** not `main`, and its git identity **is** configured locally (Szymon / szymon@protopia.tech) — no need to set it. **The editable checkout is the marketplace path above; `~/.claude/plugins/cache/trip-tools/…` is a cache copy — edit the marketplace one.** The Korea+Taiwan verdict conclusions were written back into `priorities` (§B2 rules 1, 3, 4, 5), `day-planning` (§C2) and `find-places` (step 4b) on 26 sie. ⚠️ An earlier note in this file gave `c:\src\trip-planner`; that path does not exist), not in this repo: skills `find-places`, `add-place`, `split-into-days`, plus the rules skills **`priorities`** (priority ladder + 1–5 ★ rubric) and **`day-planning`** (day-shape & clustering). Enable via `.claude/settings.local.json`. |
...
✅ **Worked example — restore the option, let the owner choose.** Olaf (🦐) had two paid viewpoints crossed off and both restored on 25 sie: the **Taipei 101 deck** and the **Miramar ferris wheel**. The planner agonised over each separately; **he settled it in seconds by picking 101**, went the next evening, and it scored **5/6** — while Miramar simply lapsed. Twice now a marked place has ended cleanly this way (the other was the Shifen lantern release, which he declined on sight). **The job is not to schedule every marked place, but to put the choice in front of the person whose marker it is.**

⚠️ **When adding markers, grep the whole file for `⏭️` collisions and report every one** — a marker landing on an already-cancelled stop is the thing most likely to be missed. Both known cases were **Olaf's**, both paid viewpoints killed by the N-Seoul-Tower-2/6 reasoning, and **both were resolved in his favour on 25 sie**: the **Taipei 101 observatory** (now scheduled 26 sie after dark) and the **Miramar ferris wheel** (now scheduled 25 sie at sunset). What broke the deadlock was the 24 sie Uber finding — the transport half of each argument evaporated. Marking the catalog entry is not enough — the ⏭️ decision row is a *separate* line and needs the flag too.
...
**⚠️ Two different scales — do not conflate them.** The ★ prefix is **1–5** and measures *planning importance*. The verdict is **1–6** and measures *how it actually was*, with **6 reserved for extraordinary** — something that broke the scale. Three 6s have been awarded: Photo SeMA / "We Are Martin Parr" (16 sie), **Tao Zhu Yin Yuan** (26 sie) — a free ten-minute architecture stop that entered the plan only as a substitute for a closed venue — and **NTMoFA** (28 sie), free, showing the Taiwanese art biennial. NTMoFA is the decisive case: a ★★★★☆ national art museum scoring 6/6 four days after a ★★★★☆ national design museum scored 1/6. Same rating, same category, same country; **the only variable was the current exhibition.** The scale was widened from 1–5 to 1–6 on 16 sie; existing verdicts kept their numerator (a 5 stayed a 5, it was not promoted), so 6 is genuinely a new top tier rather than a renaming of the old one.

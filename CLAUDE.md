...
**🐞 = życzenie Jadzi (Jadwiga's own wish-list marker).** 🐞 is a **personal marker, not a type-tag** — it means *Jadzia herself asked for this place*, collected from her WhatsApp replies to the daily `brief.txt` („Zależy mi” / „JA BARDZO BARDZO CHCĘ”). It goes **last in the emoji group**, after the type/extra tags, before the name: `- [★★★★☆ 🏛️🧒🐞 Place Name](url)`. Like every other tag it must stay **identical across `places.md` and every `days/*.md`** row for that place. It is orthogonal to both the ★ planning importance and the 1–6 verdict — it never changes either one; a 🐞 place keeps whatever stars it earned on the merits.

⚠️ **Do NOT drop a 🐞 place when rescheduling.** When a day is overloaded, a variant gets rewritten, or a leg gets re-planned, take the cuts from stops **without** 🐞. Moving a 🐞 place to another day is fine (keep the `➡️ Przeniesione z <date>` label); silently removing it from the trip is not. If a 🐞 place genuinely cannot survive (closed, unreachable, day impossible), say so explicitly and offer a replacement — don't just delete the row. Current 🐞 set (13 places, all Taiwan, Dni 13–19): Thermal Valley, Spring City Resort, Beitou Public Library, Jiufen Old Street, Yehliu Geopark, National Taiwan Science Education Center, Donguri Republic, Taiwan Design Museum, NTMoFA, National Museum of Natural Science, Tainan Art Museum, Pier-2 Art Center, KMFA. The key is canonical at the top of `places.md`; the site legend group is `Znacznik osobisty` in `LEGEND_GROUPS` (`docs/app.js`).
...
- **🐞 places survive every replan.** A place tagged 🐞 is Jadzia's own request — when trimming or re-planning a day, cut from the non-🐞 stops first. Reschedule a 🐞 place, never delete it; if it truly can't fit, flag it out loud instead of dropping it quietly. See the 🐞 convention above.
